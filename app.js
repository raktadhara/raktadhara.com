const SUPABASE_URL = 'https://pwayhjaubudecfbacjjb.supabase.co';
        ...
        const I18n = {
            currentLang: 'en',
            toggle: () => {
                I18n.currentLang = I18n.currentLang === 'en' ? 'bn' : 'en';
                document.getElementById('langLabel').innerText = I18n.currentLang === 'en' ? 'বাংলা' : 'English';
                if(I18n.currentLang === 'bn') document.body.classList.add('lang-bn');
                else document.body.classList.remove('lang-bn');
                document.querySelectorAll('[data-en]').forEach(el => { el.innerText = el.getAttribute(`data-${I18n.currentLang}`); });
            }
        };
                ...
                for (let i = 0; i < OfflineSync.queue.length; i++) {
                    ...
                    try {
                        ...
                        for(let m of item.membersPayload) {
                            m.household_id = hData.id;
                            if(m.id) {
                                await supabaseClient.from('family_members').update(m).eq('id', m.id);
                            } else {
                                inserts.push(m);
                            }
                        }
        ...
        const Auth = {
            ...
            executeLogin: (userObj) => {
                if(!userObj) return;
                activeUser = userObj;
                sessionStorage.setItem('rd_user', JSON.stringify(userObj));
            ...
            logout: () => { sessionStorage.removeItem('rd_user'); window.location.reload(); },
            checkSession: () => { const s = sessionStorage.getItem('rd_user'); if(s) Auth.executeLogin(JSON.parse(s)); }
        };
        ...
        const AdminLogic = {
            ...
            loadDetailedDatabaseExplorer: async () => {
                ...
                try {
                    const { data, error } = await supabaseClient.from('households').select(`id, house_uid, para_locality, survey_status, family_head_name, contact_number, member_count, geo_villages(village_name), family_members(full_name, relation_to_head, age, gender, blood_group, mobile, donor_availability, last_donation_date, deferral_until)`).order('created_at', {ascending: false}).limit(50);
                    if(error) throw error; exportDataForCSV = data || []; 
                ...
                } catch(e) { container.innerHTML = '<div style="color:red; padding:20px;">Database Error.</div>'; }
            },
        ...
        const HospitalLogic = {
            ...
            addBed: async (e) => {
                ...
                try {
                    const {error} = await supabaseClient.from('hospital_beds').insert([payload]);
                    if(error) throw error; UI.toast("Bed Registered."); e.target.reset(); document.getElementById('modalAddBed').style.display='none'; HospitalLogic.loadBeds();
                } catch(err) { UI.toast("Failed to add bed.", 'error'); }
            },
            deleteBed: async (id) => {
            ...
            acceptSOS: async (id) => {
                try {
                    const {error} = await supabaseClient.from('sos_tickets').update({status: 'Claimed', claimed_by: activeUser.id}).eq('id', id);
                    if(error) throw error; UI.toast("Ticket Claimed! Dispatch initiated.", "success"); HospitalLogic.loadSOS();
                } catch(e) { UI.toast("Claim failed.", "error"); }
            },
        ...
        const DoctorLogic = {
            ...
            flagAdverse: async (id) => {
                ...
                try {
                    const {error} = await supabaseClient.from('family_members').update({adverse_reaction_flag: true, deferral_until: '9999-12-31', deferral_reason: 'Doctor Adverse Reaction Flag', donor_availability: 'Permanently Ineligible'}).eq('id', id);
                    if(error) throw error; UI.toast("Patient locked for review.", "error");
                } catch(e) { UI.toast("Failed to lock.", "error"); }
            },
            raiseSOS: async (e) => {
                e.preventDefault();
                ...
                try {
                    const {error} = await supabaseClient.from('sos_tickets').insert([{ doctor_id: activeUser.id, patient_name: pat, blood_group: bg, units: parseInt(unt), urgency: urg }]);
                    if(error) throw error; UI.toast("Code Red Encrypted & Broadcasted!"); e.target.reset(); DoctorLogic.loadSOS();
                } catch(err) { UI.toast("Transmission Failed", 'error'); }
            },
            loadSOS: async () => {
        ...
        const PathologyLogic = {
            ...
            markVerified: async (id) => {
                ...
                try {
                    const { error } = await supabaseClient.from('family_members').update({ blood_group: bg, blood_verified: true, verification_doc_type: proof }).eq('id', id);
                    if(error) throw error; UI.toast("Phenotype Locked!"); document.getElementById('pathMobile').value = ''; document.getElementById('pathResults').style.display = 'none';
        ...
        const CampLogic = {
            ...
            loadCamps: async () => {
                try {
                    const { data, error } = await supabaseClient.from('camps').select('*').eq('organizer_id', activeUser.id).order('event_date', {ascending: false});
                    if(!error && data) { CampLogic.camps = data; CampLogic.switchTab('Live'); }
                } catch(e) {}
            },
            switchTab: (status) => {
            ...
            createCamp: async (e) => {
                ...
                try {
                    const {error} = await supabaseClient.from('camps').insert([payload]);
                    if(error) throw error; UI.toast("Event Orchestrator Initialized"); document.getElementById('modalCreateCamp').style.display='none'; e.target.reset(); CampLogic.loadCamps();
                } catch(err) { UI.toast("Initialization failed", "error"); }
            },
            openWorkspace: (id) => {
            ...
            logManualWalkIn: async (e) => {
                e.preventDefault();
                const name = document.getElementById('mCampName').value;
                const mob = document.getElementById('mCampMobile').value;
                const bg = document.getElementById('mCampBg').value;
                try {
                    const today = new Date().toISOString().split('T')[0];
                    const payload = { full_name: name, mobile: mob, blood_group: bg, last_donation_date: today, donor_availability: 'Temporarily Ineligible', triage_passed: true };
                    const { data, error } = await supabaseClient.from('family_members').insert([payload]).select().single();
                    if(error) throw error;
                    
                    if(CampLogic.activeCampId) {
                        await supabaseClient.from('camp_donations').insert([{ camp_id: CampLogic.activeCampId, member_id: data.id, bag_uid: 'WALK-IN-MANUAL' }]);
                    }
                    UI.toast(`Manual donation saved for ${name}. Surveyor auto-fetch enabled.`, 'success');
                    e.target.reset();
                    CampLogic.printCertificate(name, bg, 'UNREGISTERED WALK-IN');
            ...
            logDonation: async (memberId, memberName) => {
                ...
                try {
                    ...
                    UI.toast(`Donation logged for ${memberName}. 90-day protocol active.`);
                    document.getElementById('campFamilyResults').innerHTML = ''; document.getElementById('campSearchInput').value = '';
            ...
            printCertificate: (donorName, bg, houseUid) => {
                ...
                doc.setFillColor(234, 88, 12); doc.rect(225, 0, 55, 30, 'F');
                doc.setFontSize(9); doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold");
                doc.text("FAMILY / HOUSE ID", 252.5, 11, {align:"center"});
                doc.setFontSize(13); doc.text(houseUid || "WALK-IN", 252.5, 20, {align:"center"});
                ...
                doc.setFontSize(11); doc.text(`Camp: ${c?c.title:'Regional Drive'}`, 30, 175); doc.text(`Date: ${new Date().toLocaleDateString()}`, 30, 185);
                doc.text("Authorized Medical Signature", 210, 185);
        ...
        const SurveyorLogic = {
            initForm: () => { 
                ...
                const row = document.createElement('div'); row.className = 'member-card'; row.id = `memberRow_hof`;
                row.innerHTML = `
                    <input type="hidden" id="memId_hof">
                    <div class="hof-badge"><i class="ph-fill ph-crown" style="font-size:14px;"></i> Head of Family</div>
                    <div style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 1.5fr; gap: 16px; margin-bottom:16px;" class="grid-2">
                        <div class="form-group"><label class="form-label">Full Name</label><input type="text" class="form-control" id="memName_hof" required></div>
                        <div class="form-group"><label class="form-label">Relation</label><select class="form-control" id="memRel_hof" required style="pointer-events:none; background:#F1F5F9; font-weight:800;"><option value="Head of Family">Head of Family</option></select></div>
                        <div class="form-group"><label class="form-label">Age</label><input type="number" class="form-control" id="memAge_hof" required min="1" max="120"></div>
                        <div class="form-group"><label class="form-label">Gender</label><select class="form-control" id="memGender_hof" required><option value="Male">Male</option><option value="Female">Female</option><option value="Other">Other</option></select></div>
                        <div class="form-group"><label class="form-label">Mobile Number</label><div style="display:flex; gap:8px;"><input type="tel" class="form-control" id="memMobile_hof" required><button type="button" class="btn btn-outline" style="padding:0 12px; width:auto;" onclick="SurveyorLogic.fetchPreReg('hof')" title="Sync DB"><i class="ph-bold ph-arrows-clockwise"></i></button></div></div>
                    </div>
                    <div style="display: grid; grid-template-columns: 1fr 1.5fr 1fr 1fr; gap: 16px; background: rgba(254,226,226,0.1); padding: 20px; border-radius: 16px; border: 1px solid #FECDD3;" class="grid-2">
                        <div class="form-group"><label class="form-label">Blood Group</label>
                            <select class="form-control" style="background:white; border-color:#FCA5A5; font-weight:800;" id="memBg_hof" required onchange="SurveyorLogic.checkO_Negative()">
                        ...
                        <div class="form-group"><label class="form-label">Availability Status</label><select class="form-control" style="background:white; font-weight:800;" id="memDonor_hof" required onchange="SurveyorLogic.toggleTriage('hof')"><option value="Available & Willing" style="color:var(--success);">Available & Willing</option><option value="Temporarily Ineligible">Temporarily Ineligible</option><option value="Permanently Ineligible" style="color:var(--danger);">Permanently Ineligible</option><option value="Unwilling / Opted Out">Unwilling / Opted Out</option></select></div>
                        <div class="form-group"><label class="form-label">Life Status</label><select class="form-control" style="background:white;" id="memLife_hof" required><option value="Alive">Alive (Active)</option><option value="Migrated">Migrated</option><option value="Deceased">Deceased</option></select></div>
                        <div class="form-group"><label class="form-label">Last Donated</label><input type="date" class="form-control" style="background:white;" id="memLastDon_hof"></div>
                    </div>
                    <div class="triage-box" id="triageBox_hof" style="display:block;">
                        ...
                        <label><input type="checkbox" id="triAge_hof"> Citizen is between 18 and 65 years old</label>
                        <label><input type="checkbox" id="triWt_hof"> Estimated weight is ≥ 45 kg</label>
                        <label><input type="checkbox" id="triSurg_hof"> No major surgery/tattoo in past 6 months</label>
                    </div>
                `;
            ...
            addMemberRow: () => {
                const container = document.getElementById('membersContainer'); const idx = Date.now();
                const relOpts = `<option value="">Relation</option><option value="Wife">Wife</option><option value="Husband">Husband</option><option value="Son">Son</option><option value="Daughter">Daughter</option><option value="Father">Father</option><option value="Mother">Mother</option><option value="Other">Other</option>`;
                const row = document.createElement('div'); row.className = 'member-card'; row.id = `memberRow_${idx}`;
                row.innerHTML = `
                    <input type="hidden" id="memId_${idx}">
                    <div style="font-size:13px; font-weight:800; color:var(--text-muted); text-transform:uppercase; margin-bottom:16px; letter-spacing:1px;"><i class="ph-bold ph-users"></i> Family Member</div>
                    <button type="button" class="remove-btn" onclick="SurveyorLogic.removeMember(${idx})"><i class="ph-bold ph-x"></i></button>
                    <div style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 1.5fr; gap: 16px; margin-bottom:16px;" class="grid-2">
                        <div class="form-group"><label class="form-label">Full Name</label><input type="text" class="form-control" id="memName_${idx}" required></div>
                        <div class="form-group"><label class="form-label">Relation</label><select class="form-control" id="memRel_${idx}" required>${relOpts}</select></div>
                        <div class="form-group"><label class="form-label">Age</label><input type="number" class="form-control" id="memAge_${idx}" required min="1" max="120"></div>
                        ...
                        <div class="form-group"><label class="form-label">Mobile Number</label><div style="display:flex; gap:8px;"><input type="tel" class="form-control" id="memMobile_${idx}" required><button type="button" class="btn btn-outline" style="padding:0 8px; width:auto;" onclick="SurveyorLogic.fetchPreReg('${idx}')" title="Sync DB"><i class="ph-bold ph-arrows-clockwise"></i></button><button type="button" class="btn btn-outline" style="padding:0 8px; width:auto;" onclick="SurveyorLogic.copyHOFMobile(${idx})" title="Copy Head Mobile"><i class="ph-bold ph-copy"></i></button></div></div>
                    </div>
                    <div style="display: grid; grid-template-columns: 1fr 1.5fr 1fr 1fr; gap: 16px; background: rgba(254,226,226,0.1); padding: 20px; border-radius: 16px; border: 1px solid #FECDD3;" class="grid-2">
                        <div class="form-group"><label class="form-label">Blood Group</label>
                        ...
                        <div class="form-group"><label class="form-label">Last Donated</label><input type="date" class="form-control" style="background:white;" id="memLastDon_${idx}"></div>
                    </div>
                ...
                `;
                container.appendChild(row); SurveyorLogic.checkO_Negative();
            },
            
            removeMember: (id) => { document.getElementById(`memberRow_${id}`).remove(); SurveyorLogic.checkO_Negative(); },
            ...
            loadRevisits: async () => {
                ...
                try {
                    const { data } = await supabaseClient.from('households').select('house_uid, family_head_name, contact_number, geo_villages(village_name)').eq('surveyor_id', activeUser.id).eq('survey_status', 'Revisit Required');
                    ...
                    container.innerHTML = data.map(h => { let vilName = h.geo_villages ? (Array.isArray(h.geo_villages) ? h.geo_villages[0]?.village_name : h.geo_villages.village_name) : 'Unknown'; return `<div class="member-card" style="border-left: 4px solid var(--warning); padding:24px;"><strong>${h.house_uid}</strong><br><span style="font-size:13px; color:var(--text-muted); font-weight:600;">Head: ${h.family_head_name} | Mob: ${h.contact_number} | ${vilName}</span></div>`; }).join('');
                } catch(e) {}
            },
            ...
            fetchPreReg: async (idx) => {
                ...
                try {
                    const {data} = await supabaseClient.from('family_members').select('*').eq('mobile', mob).is('household_id', null).single();
                    if(data) {
                        ...
                        UI.toast("Auto-synced walk-in donor details!", "success");
                        SurveyorLogic.checkO_Negative();
                        SurveyorLogic.toggleTriage(idx);
                    } else {
                        UI.toast("No unlinked camp record found for this number.", "warning");
                    }
                } catch(e) { UI.toast("Search complete. Proceed with fresh entry.", "info"); }
            },
            ...
            submitHousehold: async (e) => {
                ...
                const btn = document.getElementById('btnSubmitSurvey'); btn.innerHTML = '<i class="ph ph-spinner ph-spin" style="font-size:24px;"></i> Synchronizing...'; btn.disabled = true;

                const houseUid = 'RD-' + Date.now().toString().slice(-6);
                const lat = parseFloat(document.getElementById('hLat').value); const lng = parseFloat(document.getElementById('hLng').value);
                const housePayload = { house_uid: houseUid, surveyor_id: activeUser.id, para_locality: document.getElementById('hLocality').value, family_head_name: document.getElementById('memName_hof').value, contact_number: document.getElementById('memMobile_hof').value, survey_status: 'Completed', member_count: document.querySelectorAll('.member-card').length, village_id: document.getElementById('hVillage').value || null, gps_lat: isNaN(lat) ? null : lat, gps_lng: isNaN(lng) ? null : lng };
                
                const membersPayload = Array.from(document.querySelectorAll('.member-card')).map((row, i) => {
                    const idx = row.id.split('_')[1]; const lastDon = document.getElementById(`memLastDon_${idx}`).value || null;
                    const isAvail = document.getElementById(`memDonor_${idx}`).value === 'Available & Willing';
                    const triPass = isAvail ? (document.getElementById(`triAge_${idx}`).checked && document.getElementById(`triWt_${idx}`).checked && document.getElementById(`triSurg_${idx}`).checked) : false;
                    const existingId = document.getElementById(`memId_${idx}`).value || null;

                    return { id: existingId, member_uid: houseUid + '-M' + (i+1), full_name: document.getElementById(`memName_${idx}`).value, relation_to_head: document.getElementById(`memRel_${idx}`).value, age: parseInt(document.getElementById(`memAge_${idx}`).value), gender: document.getElementById(`memGender_${idx}`).value, blood_group: document.getElementById(`memBg_${idx}`).value, mobile: document.getElementById(`memMobile_${idx}`).value, donor_availability: document.getElementById(`memDonor_${idx}`).value, last_donation_date: lastDon, life_status: document.getElementById(`memLife_${idx}`).value, triage_passed: triPass, village_id: housePayload.village_id };
                });

                if(!navigator.onLine) { OfflineSync.saveLocally({ housePayload, membersPayload }); } 
                else {
                    try {
                        ...
                        UI.toast('Vector Synchronized!', 'success');
                    } catch(err) { UI.toast("DB Reject: " + err.message, 'error'); }
                }

                document.getElementById('qrText').innerText = houseUid; document.getElementById('qrImage').src = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${houseUid}`; document.getElementById('qrModal').style.display = 'flex';
                e.target.reset(); SurveyorLogic.initForm();
                let count = parseInt(document.getElementById('myHouseCount').innerText) || 0; document.getElementById('myHouseCount').innerText = count + 1;
                btn.innerHTML = 'Save & Synchronize Household <i class="ph-bold ph-cloud-arrow-up" style="font-size:24px;"></i>'; btn.disabled = false;
            }
        };
        // ========================================================
